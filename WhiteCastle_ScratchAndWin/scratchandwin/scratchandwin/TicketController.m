...
- (void) viewWillAppear:(BOOL)animated
{
    ...
    if ( self.state == 0 )
    {
        UIImage *imgg = [ UIImage imageNamed:@"iphone-scratchticket-unscratched-90" ];
...
-(void) show_results: (id)obj
{
    ...
    if (self.winner)
    {
        UIImage *img = [ UIImage imageNamed:@"iphone-winner-bbq-pulled-pork" ];
    ...
    {
        UIImage *img = [ UIImage imageNamed:@"iphone-not-a-winner" ];
        self.results_iv.image = img;
        self.results_iv.hidden = NO;
        self.state = 1;
    }
